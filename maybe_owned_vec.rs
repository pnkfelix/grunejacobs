...
impl<'a,T:Copy> MaybeOwnedVector<'a,T> {
    pub fn copy_at(&'a self, idx: uint) -> T {
        match self {
            &Growable(ref v) => *v.get(idx),
            &Borrowed(ref v) => v[idx],
        }
    }
}

impl<'a,T:Clone> MaybeOwnedVector<'a,T> {
    pub fn clone_at(&'a self, idx: uint) -> T {
        match self {
            &Growable(ref v) => v.get(idx).clone(),
            &Borrowed(ref v) => v[idx].clone(),
        }
    }
}
...
impl<'a,T> MaybeOwnedVector<'a,T> {
    ...
    pub fn len(&'a self) -> uint {
        match self {
            &Growable(ref v) => v.len(),
            &Borrowed(ref v) => v.len(),
        }
    }
    pub fn get(&'a self, idx: uint) -> &'a T {
        match self {
            &Growable(ref v) => v.get(idx),
            &Borrowed(ref v) => &v[idx],
        }
